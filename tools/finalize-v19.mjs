...
// Make the unavoidable short-tail fallback visible instead of silently looking like a full gap.
...
app=app.replace(`<div class="statusline">\${reviewing ? '修改历史判断后会重新计算当天队列和 FSRS 状态。' : '只播放但没判断的词不产生学习记录；退出后会尽量从它继续。'}</div></div></main>\`;`, `\${gapNotice}<div class="statusline">\${reviewing ? '修改历史判断后会重新计算当天队列和 FSRS 状态。' : '只播放但没判断的词不产生学习记录；退出后会尽量从它继续。'}</div></div></main>\`;`);
...
app=app.replace(`<div class="statusline">不自动判中文同义词对错；熟悉/不熟悉仍然作用于同一个单词历史。</div>`, `\${gapShortfall?\`<div class="statusline">队列已到尾部，可用间隔词不足，还差约 \\${gapShortfall} 个；仍需重新判断，不会自动算通过。</div>\`:''}<div class="statusline">不自动判中文同义词对错；熟悉/不熟悉仍然作用于同一个单词历史。</div>`);
write('src/app.js',app);

// Regression: reopening must credit intervening real judgments already completed after the last target attempt.
