...
`test('retry waits for five completed other cards, not wall clock',()=>{const s=state();add(s,'a','bad');const p={date:'2026-08-14',newIds:['a','b','c','d','e','f','g'],reviewIds:[]};const q=createRetrySession(s,p,'listen');for(let i=0;i<5;i++){const id=pickNext(q);assert.notEqual(id,'a');add(s,id,'good','listen',10+q.turn);finishCurrent(q,'good',s);}assert.equal(pickNext(q),'a');});`);
